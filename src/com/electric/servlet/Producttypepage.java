...
/**
 * Servlet implementation class ProductServlet_findListByTypeId
 */
@WebServlet("/Producttypepage")
public class Producttypepage extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		myPage.setRowCount(productService.findListByTypeId(typeId).size());//某类型下新闻的记录数作为分页对象的总记录数
		request.setAttribute("productList", productService.findPageList(myPage,typeId));
		request.setAttribute("productTypeList", productTypeService.findAll());//传递新闻类型列表到页面中
		request.setAttribute("myPage", myPage);//传递分页对象到页面
		
		request.setAttribute("productType", productTypeService.findById(typeId));//多传递一个新闻类型对象到页面//////
